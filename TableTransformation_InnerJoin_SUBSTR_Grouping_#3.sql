...
SELECT '
Returns the summation for the total day sum for each category, for a single employee grouped by month.
';
...
FROM (
	...
	WHERE
		...
		l != 0 OR
		m != 0) AND
		SS_Installations.data_date < '2017-04-01' AND
		employee.clk_no = '0912') Name_Data_AllDaySums 
		
GROUP BY name, SUBSTR(data_date,1,7)

ORDER BY name ASC, SUBSTR(data_date,1,7) ASC;