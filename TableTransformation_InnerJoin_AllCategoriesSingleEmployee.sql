...
SELECT '';
SELECT 'Returns the total summation from each category for each individual employee which is sorted in descening order by total hours worked.';
SELECT '';
...
SELECT
	...
	SUM(virtual_table.a) + SUM(virtual_table.b) + SUM(virtual_table.c) + SUM(virtual_table.d)
		...
		+ SUM(virtual_table.m) as 'Total_Summation'
